...
async function enemyShot(epoch){
  ...
  function getUiAudio(){
    try{
      ...
      return uiClick.ctx||(uiClick.ctx=new C({latencyHint:'interactive'}));
    }catch(e){return null}
  }
  function uiClick(){
    const c=getUiAudio();if(!c)return;
    const fire=()=>{
      try{
        const t=c.currentTime+.003;
        const snap=c.createOscillator(),sg=c.createGain();
        snap.type='square';snap.frequency.setValueAtTime(1320,t);snap.frequency.exponentialRampToValueAtTime(650,t+.032);
        sg.gain.setValueAtTime(.16,t);sg.gain.exponentialRampToValueAtTime(.001,t+.045);
        snap.connect(sg);sg.connect(c.destination);snap.start(t);snap.stop(t+.05);
        const body=c.createOscillator(),bg=c.createGain();
        body.type='sine';body.frequency.setValueAtTime(260,t+.012);body.frequency.exponentialRampToValueAtTime(120,t+.075);
        bg.gain.setValueAtTime(.12,t+.012);bg.gain.exponentialRampToValueAtTime(.001,t+.085);
        body.connect(bg);bg.connect(c.destination);body.start(t+.012);body.stop(t+.09);
      }catch(e){}
    };
